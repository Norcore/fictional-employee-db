...
const SearchEmployees = () => {
    ...
    // GET employees
    useEffect(() => {
        fetch(`http://${process.env.REACT_APP_BACKEND_URL}:3001/api/employees/candidates?filter=${filterName}`)
    ...
    const handleSimilarEmployees = (employee) => {
        ...
        fetch(`http://${process.env.REACT_APP_BACKEND_URL}:3001/api/employees/similaremployees?position=${employee.position}&level=${employee.level}`)
        .then(response => response.json())
        .then(data => setEmployees(data))
