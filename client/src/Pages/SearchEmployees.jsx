...
const SearchEmployees = () => {
    ...
    // GET employees
    useEffect(() => {
        fetch(`http://localhost:8080/api/employees/candidates?filter=${filterName}`)
    ...
    const handleSimilarEmployees = (employee) => {
        ...
        fetch(`http://localhost:8080/api/employees/similaremployees?position=${employee.position}&level=${employee.level}`)
        .then(response => response.json())
        .then(data => setEmployees(data))
