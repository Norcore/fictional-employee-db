...
const fetchEmployees = () => {
  return fetch("http://procecc.env.BACKEND_URL/api/employees").then((res) => res.json());
};

const deleteEmployee = (id) => {
  return fetch(`http://procecc.env.BACKEND_URL/api/employees/${id}`, { method: "DELETE" }).then((res) =>
    res.json()
  );
};

  const updateEmployeePresence = (id, present) => {
    return fetch(`http://procecc.env.BACKEND_URL/api/employees/${id}/missing`, {
      method: 'PATCH',
      headers: {
