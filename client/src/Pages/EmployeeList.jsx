...
const fetchEmployees = () => {
  return fetch("http://server:3001/api/employees").then((res) => res.json());
};

const deleteEmployee = (id) => {
  return fetch(`http://server:3001/api/employees/${id}`, { method: "DELETE" }).then((res) =>
    res.json()
  );
};

  const updateEmployeePresence = (id, present) => {
    return fetch(`http://server:3001/api/employees/${id}/missing`, {
      method: 'PATCH',
      headers: {
